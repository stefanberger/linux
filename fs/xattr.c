			  ...
			  bool getsize)
{
	...
	while (s_off < size) {
		...
		if (xattr_is_userns_supported(name, false) >= 0) {
			/* security.foo of the host 'shines through' */
			newname = name;
		} else {
		   ...
		   size_t xattr_size, gfp_t flags)
{
	...
	do {
		...
		error = handler->get(handler, dentry, inode, nname, NULL, 0);
		if (error > 0) {
			if (!value || (error > xattr_size)) {
				value = krealloc(*xattr_value, error + 1,
						 flags);
				if (!value) {
					error = -ENOMEM;
					goto out;
			...
			error = handler->get(handler, dentry, inode, nname,
					     value, error);
			*xattr_value = value;
	...
	} while ((error == -ENODATA) && userns && userns_supt_xattr);
