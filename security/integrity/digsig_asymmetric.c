...
static int asymmetric_verify_common(const struct key *keyring,
			     ...
			     struct ima_digest_data *hash)
{
	...
	file_id_data = (const u8 *)&file_id;
	file_id_size = sizeof(file_id) - (HASH_MAX_DIGESTSIZE - digest_size);

	rc = crypto_shash_init(desc) ?:
	     crypto_shash_update(desc, file_id_data, file_id_size) ?:
	     crypto_shash_final(desc, hash->digest);
					 ...
					 const u8 *digest)
{
	...
	if (hdr->type != IMA_VERITY_DIGSIG && hdr->type != EVM_IMA_XATTR_DIGSIG)
		return -EINVAL;
			 ...
			 const char *data, int datalen, u8 algo)
{
	...
	if (!strncmp(pk->pkey_algo, "mldsa", 5)) {
		...
		rc = calc_file_id_hash(hdr->type, algo, data,
